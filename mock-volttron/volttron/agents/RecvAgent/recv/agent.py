...
from mock_volttron.vip.agent import Agent, Core, PubSub
from mock_volttron.messaging.headers import TIMESTAMP
from mock_volttron.scheduling import periodic
from mock_volttron.utils import (get_aware_utc_now, format_timestamp, setup_logging, load_config, Unreachable)
import mock_volttron.utils as utils
...
class RecvAgent(Agent):
    ...
    def on_match_topic(self, peer, sender, bus, topic, headers, message):
        ...
        # Send the structured raw data to the UI server, instead of the flattened data 
...
def main(argv=sys.argv):
    ...
    try:
        utils.vip_main(RecvAgent, version=__version__, argv=argv)
