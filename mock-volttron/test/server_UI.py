...
def set_temperature_setpoints(control_signals): #(y_, system_id, control_signals):
    """`    
    ...
    json_object = json.dumps(control_signals, default=str) 
    ...
    try:        
        ...
        if result['status'] == 200:
            return {'status':200, 'message':'Success', 'payload':None} #, y_
    except: 
    ...
    def post(self):
        ...
        if req.params.authentication:
            ...
            if req.method == "get_temperature_setpoints":
                ...
                payload = set_temperature_setpoints(y, req.id, req.params.data)
                # y = y_
            else:
